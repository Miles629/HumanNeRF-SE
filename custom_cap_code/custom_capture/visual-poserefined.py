...
import json
import cv2
# python ./scripts/visual-poserefined.py  ./data/custom/lgp1 --visualize_smpl  n 0
try:
    import smplx
    import torch
except ImportError:
...
def visualize(root_dir, seq_name=None, view_id=0, 
    ...
    for i in range(0,1000,skip):
        ...
        if visualize_smpl:
            ...
            for i,point in enumerate(proj_vertices):
                p = (i+1)/6891
                p = int(255*p)
                c = (p,p,p)
                color = cv2.circle(color, point, 1, c, thickness=-1)
            ...
            output_image_path = osp.join(root_dir,f"output-optimized/output_{frame_id:06d}.png")
