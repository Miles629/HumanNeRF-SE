...
def optimize(optimizer, closure, max_iter=10):
    ...
    for i in pbar:
        ...
        pbar.set_postfix_str("loss: {:.6f}".format(loss.detach().cpu().numpy()))
...
def main(root, gender, keypoints_threshold, use_silhouette, downscale=1):
    ...
    for k, v in smpl_params.items():
        if k == "thetas":
            ...
            params[k] = nn.Parameter(tensor[None]).requires_grad_(False)
            print("betas,shape:",params[k].shape)
            # params[k] = tensor[None]
        else:
            print("else k!=thetas or betas,k =",k)
            tensor = torch.from_numpy(v).clone().to(DEVICE)
            params[k] = nn.Parameter(tensor)
            print("shape-",k,params[k].shape)

    

    body_model = smplx.SMPL("./data/SMPLX/smpl", gender=gender).requires_grad_(False)
    # body_model = smplx.SMPL("./data/SMPLX/smpl", gender=gender)
    body_model.to(DEVICE)

    # optimize with keypoints
    optimizer = torch.optim.Adam(params.values(), lr=1e-5)
    def closure():
        optimizer.zero_grad()
        ...
        loss.backward()
        return loss
    optimize(optimizer, closure, max_iter=MAXITER)
