...
#         code = self.code
#         c = torch.full((6890,24),1).to(code)
#         code = torch.cat([code,c],dim=1)
...
# #         pts_id = torch.tensor(pts_id,dtype=torch.int32)
#         for i in range (1,25):
#             idx = torch.full([vertices.shape[0],1], i).to(vertices)
#             pts_idx = torch.cat([idx, vertices], dim=1)
# #             pts_idx = torch.tensor(pts_idx,dtype=torch.int32)
#             pts_id = torch.cat([pts_id,pts_idx],dim=0)
...
#         print("xyzc_features",xyzc_features.shape)
...
class SparseConvNet(nn.Module):
    def __init__(self):
        ...
        self.one_conv1 = self.single_conv(25,64,'conv1')
        self.one_conv2 = self.single_conv(64,128,'conv2')
        self.one_conv3 = self.single_conv(128,64,'conv3')
        self.one_conv4 = self.single_conv(64,25,'conv4')
...
#         exit()
        ...
        net = self.one_conv1(x)
        net = self.one_conv2(net)
        net = self.one_conv3(net)
        net = self.one_conv4(net)
        
        
        netout.append(net.dense())
        return netout
    def single_conv(self,in_channels, out_channels, indice_key=None):
