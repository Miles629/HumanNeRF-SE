        ...
        for sub in subjects:
            ...
            dataset_attrs.update({
                f"zju_{sub}_train": {
                    ...
                    "dataset_path": f"dataset/zju_mocap/{sub}_16split_train",
                    # "dataset_path": f"dataset/zju_mocap/{sub}",
                    # "dataset_path": f"dataset/zju_mocap/{sub}_fewshot_train",
                    # "dataset_path": f"dataset/zju_mocap/{sub}_ct_train",
...
#                     "dataset_path": f"dataset/zju_mocap/{sub}_split_eval",
                    ...
                    # "dataset_path": f"dataset/zju_mocap/{sub}_fewshot_eval",
                    "dataset_path": f"dataset/zju_mocap/{sub}_41_eval",
#                     "dataset_path": f"dataset/zju_mocap/{sub}_novelpose",
    ...
    if cfg.category == 'human_nerf' and cfg.task == 'custom':
        subs = ['lgp','xjy','mcy']
        # print("1DatasetArgs, peoplesnapshot")
        for sub in subs:
            dataset_attrs.update({
                f"{sub}_train": {
                    "dataset_path": f"dataset/custom/{sub}1/",
                    "keyfilter": cfg.train_keyfilter,
                    "ray_shoot_mode": cfg.train.ray_shoot_mode,
                },
                f"{sub}_test": {
                    "dataset_path": f"dataset/custom/{sub}2",
                    "keyfilter": cfg.test_keyfilter,
                    "ray_shoot_mode": 'image',
                    "src_type": 'custom'
                },
