...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.canonical_joints, self.canonical_bbox, self.vertices = \
    ...
    def load_canonical_joints(self):
        ...
        vertices = cl_joint_data['t-v'].astype('float32')

        return canonical_joints, canonical_bbox,vertices
    ...
    def __getitem__(self, idx):
        ...
        if 'motion_bases' in self.keyfilter:
            ...
            x = 32
            out_sh = (out_sh | (x - 1)) + 1
            vertices = np.round((self.vertices - dst_bbox['min_xyz']) / cfg.voxelsize).astype(np.int32)
            results.update({
                'dst_Rs': dst_Rs,
                'dst_Ts': dst_Ts,
                'cnl_gtfms': cnl_gtfms,
