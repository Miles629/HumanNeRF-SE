...
class Dataset(torch.utils.data.Dataset):
    ...
    def load_train_mesh_infos(self):
        ...
        for frame_name in mesh_infos.keys():
            # print(mesh_infos[frame_name].keys())
            # exit()
    ...
    def query_dst_skeleton(self):
        # print(self.train_mesh_info.keys())
        return {
            ...
            'vertices': self.train_mesh_info['vertices'].astype('float32')
    ...
    def __getitem__(self, idx):
        ...
        if 'motion_bases' in self.keyfilter:
            ...
            out_sh = np.ceil((dst_bbox['max_xyz'] - dst_bbox['min_xyz']) / [0.005, 0.005, 0.005]).astype(np.int32)
            x = 32
            out_sh = (out_sh | (x - 1)) + 1
            vertices = np.round((vertices - dst_bbox['min_xyz']) / [0.005, 0.005, 0.005]).astype(np.int32)
