...
class Dataset(torch.utils.data.Dataset):
    ...
    def load_image(self, frame_name, bg_color):
        # print('frame_name',frame_name,'bg_color',bg_color)
        ...
        if frame_name in self.cameras and 'distortions' in self.cameras[frame_name]:
            ...
            # print("cameras",self.cameras,"frame_name",frame_name,"orig_img",orig_img.shape,"K",K.shape,"jibian:",D.shape)
            # exit()
    ...
    def __getitem__(self, idx):
        ...
        vertices = dst_skel_info['vertices']
        ...
        if 'motion_bases' in self.keyfilter:
            ...
            cnl_gtfms = get_canonical_global_tfms(
                            self.canonical_joints)
            # print('dst_bbox',type(dst_bbox['min_xyz']),dst_bbox['max_xyz'])
            bbox = np.concatenate(([dst_bbox['min_xyz']],[dst_bbox['max_xyz']]))
            out_sh = np.ceil((dst_bbox['max_xyz'] - dst_bbox['min_xyz']) / cfg.voxelsize).astype(np.int32)
            x = 32
            out_sh = (out_sh | (x - 1)) + 1
            vertices = np.round((vertices - dst_bbox['min_xyz']) / cfg.voxelsize).astype(np.int32)
