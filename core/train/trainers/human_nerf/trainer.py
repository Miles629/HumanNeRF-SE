...
class Trainer(object):
    ...
    def get_loss(self, net_output, 
        ...
        cnl_rgb = _unpack_imgs(net_output['rgb'], patch_masks, bgcolor, targets, div_indices)
        losses = self.get_img_rebuild_loss(loss_names, cnl_rgb,targets)

        train_losses = [
            weight * losses[k] for k, weight in lossweights.items()
        ]
        ob_rgb = _unpack_imgs(net_output['ob_rgb'], patch_masks, bgcolor,targets, div_indices)
        ob_losses = self.get_img_rebuild_loss(loss_names, ob_rgb, targets)
        ob_train_losses = [
            weight * ob_losses[k] for k, weight in lossweights.items()
        ]
        consistency = img2mse(cnl_rgb, ob_rgb)

        return sum(train_losses), \
               {loss_names[i]: train_losses[i] for i in range(len(loss_names))},\
                sum(ob_train_losses),\
                {'ob_'+loss_names[i]: ob_train_losses[i] for i in range(len(loss_names))},\
                    consistency
    def get_loss_origin(self, net_output, 
                    patch_masks, bgcolor, targets, div_indices):
            ...
            return sum(train_losses), \
                {loss_names[i]: train_losses[i] for i in range(len(loss_names))}
    def get_loss_mse(self, net_output, 
                    patch_masks, bgcolor, targets, div_indices):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            if self.iter % cfg.train.log_interval == 0:
                ...
                log_str = log_str.format(
                    ...
                    self.timer.log(),
                    loss_str)
                # log_str = 'Epoch: {} [Iter {}, {}/{} ({:.0f}%), {}] {},consistency:{}'
                # log_str = log_str.format(
                #     epoch, self.iter,
                #     batch_idx * cfg.train.batch_size, len(train_dataloader.dataset),
                #     100. * batch_idx / len(train_dataloader), 
